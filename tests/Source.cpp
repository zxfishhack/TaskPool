...
void FunctionTask(int ud) {
	...
	for(int i=0; i<ud; i++) {
		Task::Promise<int> ret = Task::async<int>(boost::bind(TestAsync, i));
		waitList.push_back(ret);
		resultList.push_back(ret);
	}
	Task::Promise<void> waitResult = Task::waitAll(waitList, 5000);
...
void FunctionTask2(int ud) {
	auto res = Task::await(Task::async<int>(boost::bind(TestAsync2, ud))).result();
